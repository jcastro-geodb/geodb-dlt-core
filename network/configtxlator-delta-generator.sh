#!/bin/bash +x
#
# This script wraps the complete process for generating the needed update-delta
# for an organization to join a channel, by means of the tool configtxlator
...
if [ -z "$OUTPUT_DELTA_FILE" ]; then
  OUTPUT_DELTA_FILE=./channels/final-update-delta-$ORG_NAME.pb
fi
...
curl -X POST --data-binary @$CONFIGTXLATOR_ARTIFACTS_DIR/config_block-delta-wrapped.json "$CONFIGTXLATOR_URL/protolator/encode/common.Envelope" > $CONFIGTXLATOR_ARTIFACTS_DIR/final-update-delta-${ORG_NAME}.pb

cp $CONFIGTXLATOR_ARTIFACTS_DIR/final-update-delta-${ORG_NAME}.pb $OUTPUT_DELTA_FILE
